...
var PELLOT_INNER_COLOR = color(200, 220, 32);
var PELLOT_COLOR = color(245, 245, 245);
var WALL_COLOR = color(150,200,175);
 ...
 JaxLocation (JaxEntity internalObject, int x, int y,
  ...
  void setWalls(boolean northWall, boolean southWall, boolean eastWall,boolean westWall){
   ...
   void drawJaxEntity(){
     ...
     { 
        //drawNorthWall
       line(x*PATH_WIDTH,this.y*PATH_WIDTH+PATH_WIDTH/2,
             x *PATH_WIDTH+ PATH_WIDTH,  y*PATH_WIDTH+PATH_WIDTH/2);
     }
     if(this.hasSouthWall)
     {
        //drawSouthWall
       
        line(x*PATH_WIDTH,this.y*PATH_WIDTH+PATH_WIDTH*1.5,
             x*PATH_WIDTH +PATH_WIDTH,  y*PATH_WIDTH+PATH_WIDTH*1.5);
     }
     if(this.hasEastWall){
         //drawEastWall
        // stroke(5, 0, 0);
         line( this.x*PATH_WIDTH +PATH_WIDTH, y*PATH_WIDTH+VERTICAL_OFFSET,
                            x*PATH_WIDTH +PATH_WIDTH, 
                            y*PATH_WIDTH+PATH_WIDTH+VERTICAL_OFFSET);
     }
     if(this.hasWestWall){
         //drawWestWall
          line( x*PATH_WIDTH, y*PATH_WIDTH+VERTICAL_OFFSET, 
                    x*PATH_WIDTH, y*PATH_WIDTH+PATH_WIDTH+VERTICAL_OFFSET);
     }
  }//drawJaxEntity
}//JaxLocation
...
class JaxPellot extends JaxEntity{
 ...
 void drawJaxEntity(){
   ...
   // fill(0,0,0);
    ...
    point(PATH_WIDTH * x+PATH_WIDTH/2,
            VERTICAL_OFFSET+y*PATH_WIDTH+PATH_WIDTH/2 );
 }
}
 ...
 */
  boolean move(JaxEntity jaxObject, int destX, int destY){
        ...
        return jaxObject.moveToPosition(destX,destY);
...
//could be on the same location
   ...
   if (keyCode === UP) {
     //1. does current wall have north wall?
    if(this.jaxMan.y>0 && !this.gridMap[this.jaxMan.x] [this.jaxMan.y].hasNorthWall)
    {
        //2nd. check to see if dest location has a south wall
         if(!this.gridMap[this.jaxMan.x] [this.jaxMan.y-1].hasSouthWall){
              this.lookAhead(this.jaxMan, this.jaxMan.x, this.jaxMan.y-1);
         }   
    }
  } else
    if (keyCode === DOWN) {
     //1. does current wall have a south wall?
       if(this.jaxMan.y<GRIDSIZE &&
            !this.gridMap[this.jaxMan.x] [this.jaxMan.y].hasSouthWall){
                
        //2nd. check to see if dest location has a northth wall
         if(!this.gridMap[this.jaxMan.x] [this.jaxMan.y+1].hasNorthWall){
              this.lookAhead(this.jaxMan, this.jaxMan.x, this.jaxMan.y+1);
         }   
       }
    } else if (keyCode === LEFT) {
       
    //1. does current wall have a west wall?
        if(  !this.gridMap[this.jaxMan.x] [this.jaxMan.y].hasWestWall)
        {
            //2nd. check to see if dest location has an east wall
             if(!this.gridMap[this.jaxMan.x-1] [this.jaxMan.y].hasEastWall){
                  this.lookAhead(this.jaxMan, this.jaxMan.x-1, this.jaxMan.y);
             }   
        }    
    } else 
    if (keyCode === RIGHT) {
         //1. does current wall have an east wall?
        if(this.jaxMan.x<GRIDSIZE &&
                !this.gridMap[this.jaxMan.x] [this.jaxMan.y].hasEastWall)
        {
            //2nd. check to see if dest location has a west wall
             if(!this.gridMap[this.jaxMan.x+1] [this.jaxMan.y].hasWestWall){
                  this.lookAhead(this.jaxMan, this.jaxMan.x+1, this.jaxMan.y);
             }   
        } 
    }
...
/*************************  Events **********************/
 ...
 void mousePressed() { 
    ...
    //toggle play mode
        if(isPlaying){
//make isPlaying false so that at the next iteration of draw() the menu will appear
           isPlaying = false;
           paused = true;
           
        }else{
            /*
             *  menu mode : The top button is always [New Game]
             *              If the game is paused, the second button will be [Resume]
             *              If it's the initial menu(game !paused),
             *                                  the second button will be [Info]
             *  
             */
             //check 1st if is inside new game button rect(127, 158, 144, 28)
           var mouseXInBounds =0  ; 
           var mouseYInBounds =0  ; 
if(infoMenuMode){// rect(301,33, 37,32,133);
      mouseXInBounds = mouseX>301 && mouseX<(301+37+144)  ; 
      mouseYInBounds = mouseY>33 && mouseY<(33+32)  ; 
    if ( mouseXInBounds &&  mouseYInBounds){
      infoMenuMode = false;
    }
}
           ...
           //check 1st if is inside new game button rect(127, 158, 144, 28)
           var mouseXInBounds = mouseX>127 && mouseX<(127+144)  ; 
           var mouseYInBounds = mouseY>158 && mouseY<(158+28)  ; 
          
            if (!infoMenuMode && mouseXInBounds &&  mouseYInBounds){
  
                jaxWorld.gameSetup();
                touchedAnEnemy = false;
                isPlaying = true;
                paused = false;
                 ...
                 //rect(127, 212, 144, 28, -10); 2nd button; 
                    mouseYInBounds = mouseY>212 && mouseY<(212+28)  ;
                    if(paused){
                        // 2nd button [resume]
                       if (mouseXInBounds &&  mouseYInBounds){
                            isPlaying = true;
                            ...
                            //rect(127, 264, 144, 28, -10); 3nd button; [info] 
                            mouseYInBounds = mouseY>264 && mouseY<(264+28)  ;
                             if (mouseXInBounds &&  mouseYInBounds){
                               // isPlaying = true;
                              //  paused = false;
                                 showInfo();
                    ...
                    }else{//initial start-up menu
                          // 2nd button [options]
                           if (mouseXInBounds &&  mouseYInBounds){
                                //show options/instructions menu
                               showInfo();
                                infoMenuMode = true;
                            }
                    }//else startup
...
// check to see if at highest level  or if time to advance to next level
   if(jaxWorld.level< MAX_LEVEL && jaxWorld.pellotCounter===0){
    ...
    if(isPlaying)
    {
         if(touchedAnEnemy)
        {
        console.log("enemyTouched");
        
            if(jaxWorld.kmanLivesRemaining>0){
                jaxWorld.kmanLivesRemaining-=1;
                //reposition Khanman at his starting point
                jaxWorld.rePositionJaxman();
                //move enemies back home too
                jaxWorld.initEnemies();
                touchedAnEnemy = false;
                //wait a few seconds and then go again
            }else{
                 isPlaying  = false;
                showMenu(isPlaying, true);
            }
          
        }else{//not touching enemy
  
                 jaxWorld.drawJaxWorld();
                 jaxWorld.moveEnemies(frameCount);
            ...
            if(infoMenuMode ){
                ...
                //show menu
                showMenu(paused,jaxWorld.isGameOver);
            }
    }
