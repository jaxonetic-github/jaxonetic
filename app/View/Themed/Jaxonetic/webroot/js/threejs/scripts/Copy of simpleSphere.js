...
var menuSphere,refractSphereCamera;

var BLOGROUND_TEXT_YOFFSET = 30;
// custom global variables
var cube, sphereMenu;
// initialization
init();
...
console.log("aqui");
  ...
  //  else
    ...
    var ambientBottomLight = new THREE.AmbientLight(0x111111);
    scene.add(ambientBottomLight);
    ...
    {
        size: 1, height: .4, curveSegments: 33,
...
function initGeometricObjects(){
    ...
    var sphereGeometry = new THREE.SphereGeometry( .4, 32,10 ); 
    ...
    **/
       ...
       // direction (normalized), origin, length, color(hex)
    var origin_of_rightarrow = new THREE.Vector3(2, 3,-5);
    ...
    // direction (normalized), origin, length, color(hex)
    var origin_of_leftarrow = new THREE.Vector3(-2, 5,-5);
    ...
    //say
    sayAt("Blog", -7 , 7, 0, -Math.PI /30,Math.PI /6,0 );
   // sayAt("G", -7 , 3, -10, -Math.PI /30,0,0 );
  
   // sayAt("Ground", -5 , 4, -5, 0,0,-Math.PI /4);
   // sayAt("u", -5.5 , 4, -10, 0,0);
    //sayAt("nd", -4.5, 4, -10, 0,0);
	////////////
	// CUSTOM //
	////////////
 ...
 *	refractSphereCamera = new THREE.CubeCamera( 0.1, 5000, 512 );
	...
	menuSphere = new THREE.Mesh( sphereGeometry, customMaterial );
	menuSphere.position.set(0, 0, -10);
	scene.add(menuSphere);
	
	refractSphereCamera.position = menuSphere.position;
	refractSphereCamera.lookAt(menuSphere.position);
    */
